...
describe('mark', () => {
	...
	testData.forEach(([transl, system]) => {
		...
		test(`testing: ${system}`, () => {
			checker(data, new Converter({ system, punctuation: 'none', format: 'mark' }), new Converter({ system, dialect: "north", punctuation: 'none', format: 'mark' }));
		});
	});
});
...
describe('number', () => {
	...
	testData.forEach(([transl, system]) => {
		const data = hanjiData.map((h, i) => [h, transl[i]]);
		test(`testing: ${system}`, () => {
			checker(data, new Converter({ system, punctuation: 'none', format: 'number' }), new Converter({ system, dialect: "north", punctuation: 'none', format: 'number' }));
		});
	});
});
...
describe('strip', () => {
	...
	testData.forEach(([transl, system]) => {
		const data = hanjiData.map((h, i) => [h, transl[i]]);
		test(`testing: ${system}`, () => {
			checker(data, new Converter({ system, punctuation: 'none', format: 'strip' }), new Converter({ system, dialect: "north", punctuation: 'none', format: 'strip' }));
		});
	});
});