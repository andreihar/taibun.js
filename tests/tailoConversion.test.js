...
test('tailoInitials', () => {
	const bilabial = ['pi', 'pho', 'môo', 'bâ'];
	checker(bilabialC.map((h, i) => [h, bilabial[i]]), c, cNorth);
	const alveolar = ['tē/tuē', 'thò', 'tsá', 'tshù', 'su', 'nāi', 'jû/lû', 'liú'];
	checker(alveolarC.map((h, i) => [h, alveolar[i]]), c, cNorth);
	const alveoloPalatal = ['tsia', 'tshiú', 'siá', 'jî/lî'];
