...
const hanjiA = ["廟尪仔", "翁某仔", "肉幼仔", "花搭仔", "微微仔", "慢慢仔", "尾蝶仔", "佇厝仔頂我咧行", "塗跤清氣矣", "阮佇厝外攏講台語", "佇公園內睏"];
...
// 	];
// 	testData.forEach(([transl, system]) => {
// 		const data = hanjiA.map((h, i) => `${h},${transl[i]}`);
// 		test(`testing: ${system}`, () => {
// 			checker(data, new Converter({ system, punctuation: 'none', sandhi: 'auto' }), new Converter({ system, dialect: "north", punctuation: 'none', sandhi: 'auto' }));
// 		});
// 	});
...
// 		const data = hanjiA.map((h, i) => `${h},${transl[i]}`);
// 		test(`testing: ${system}`, () => {
// 			checker(data, new Converter({ system, punctuation: 'none', sandhi: 'exlLast' }), new Converter({ system, dialect: "north", punctuation: 'none', sandhi: 'exlLast' }));
// 		});
// 	});
// });

// describe('add inclLast', () => {
// 	const testData = [
