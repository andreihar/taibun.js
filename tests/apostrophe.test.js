...
const hanjiData = ["一概", "三層", "不屑", "朋友", "海蟹", "癮甲", "草笠仔"];
...
describe('default', () => {
	...
	testData.forEach(([transl, system]) => {
		...
		test(`testing: ${system}`, () => {
			checker(data, new Converter({ system, punctuation: 'none', apostrophe: true }), new Converter({ system, dialect: "north", punctuation: 'none', apostrophe: true }));
		});
	});
});
...
describe('delimiter', () => {
	...
	testData.forEach(([transl, system]) => {
		const data = hanjiData.map((h, i) => [h, transl[i]]);
		test(`testing: ${system}`, () => {
			checker(data, new Converter({ system, punctuation: 'none', apostrophe: true, delimiter: '' }), new Converter({ system, dialect: "north", punctuation: 'none', apostrophe: true, delimiter: '' }));
		});
	});
});

describe('delimiter number', () => {
	const testData = [
	...
	testData.forEach(([transl, system]) => {
		const data = hanjiData.map((h, i) => [h, transl[i]]);
		test(`testing: ${system}`, () => {
			checker(data, new Converter({ system, punctuation: 'none', apostrophe: true, delimiter: '', format: 'number' }), new Converter({ system, dialect: "north", punctuation: 'none', apostrophe: true, delimiter: '', format: 'number' }));
		});
	});
});

describe('delimiter strip', () => {
	const testData = [
	...
	testData.forEach(([transl, system]) => {
		const data = hanjiData.map((h, i) => [h, transl[i]]);
		test(`testing: ${system}`, () => {
			checker(data, new Converter({ system, punctuation: 'none', apostrophe: true, delimiter: '', format: 'strip' }), new Converter({ system, dialect: "north", punctuation: 'none', apostrophe: true, delimiter: '', format: 'strip' }));
		});
	});
});